...
**1. `~/.game2-cli/`** - Deployment metadata
...
- Contains: LevelDB database with encrypted private contract state
- Purpose: Stores your local state for interacting with deployed contracts
- Git: **Must be ignored** (already in `.gitignore`)
- Backup: Optional - can be regenerated by re-syncing with the blockchain
- Delete: Safe to delete if you want to re-sync from scratch
